...
async function removeBuildDirectory(directory, packageRoot) {
    ...
    const allowed = ["dist", "src", "build/src"].map((name) => path.resolve(packageRoot, name));
    if (!allowed.includes(target)) throw new Error(`Refusing to remove unexpected build directory: ${target}`);
    await retryFileOperation(() => fs.rm(target, {
        recursive: true,
        force: true,
        maxRetries: 3,
        retryDelay: 100,
...
async function minifyFiles(directory, format) {
    if (!["esm", "cjs"].includes(format)) throw new Error(`Unsupported output format: ${format}`);
    const entries = await fs.readdir(directory, { withFileTypes: true });
    for (const entry of entries) {
        const file = path.join(directory, entry.name);
        if (entry.isDirectory()) {
            await minifyFiles(file, format);
            ...
            try {
                await fs.writeFile(temporary, code, { flag: "wx", mode: (await fs.stat(file)).mode });
